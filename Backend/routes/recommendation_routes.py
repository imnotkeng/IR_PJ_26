...
def generate_ml_recommendations(bookmarks, limit=10):
    ...
    # 1. Get recipe text for these bookmarks to build User Profile
    bookmark_texts = []
    for b in bookmarks:
    ...
    # 2. Create User Profile Matrix (u_chunk)
    ...
    # 3. Find the top 5 words to ask Elasticsearch for candidates
    feature_names = vectorizer.get_feature_names_out()
    top_indices = user_profile_array.A[0].argsort()[-5:][::-1]
    top_words = [feature_names[i] for i in top_indices]
    search_query = " ".join(top_words)

    # 4. Ask Elasticsearch for 50 candidate recipes (50 is plenty and keeps it fast)
    ...
    for hit in candidate_res["hits"]["hits"]:
        item = hit["_source"]
        # Skip recipes the user already bookmarked!
